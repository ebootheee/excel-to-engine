/**
 * Regression test for the COL$ROW mixed-reference lexer bug (formula_ast.rs).
 *
 * Before the fix, a relative-column/absolute-row reference like `DB$7` (or `B$1`)
 * lexed as Ident("DB") → StringLit → JS `expr * "DB"` = NaN, and the remainder of
 * the formula was silently dropped — 240,973 such cells on the real Outpost A-1
 * model (ADR-026 Wave-2 gate finding). This asserts `COL$ROW` now parses as a real
 * cell reference (abs row), the range form works, and no `* `COL`` string-multiply
 * survives. Skips if the rust-parser isn't built.
 *
...
try {
  ...
  const strMul = mod.match(/\* `[A-Z]{1,3}`/g) || [];
  assert(strMul.length === 0, `no `+'`* `COL``'+` string-multiplies remain (found ${strMul.length})`);
