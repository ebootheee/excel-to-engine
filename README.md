...
| **`dependency-graph.json`** *(debug)* | `{ format:"cell-dependency-edges-v3", edges: cell → [cells/ranges it reads] }` | Cell-level forward edges — the raw material for the `dependsOnNamedInputs` / `affectsOutputs` closures above. Ranges are kept as **compact tokens** (`Sheet!A1:B10`), not expanded to interior cells: full expansion was 37 GB / ~7 min on the real models and OOM-killed the closure-baking step (#32); the compact form is ~0.5 GB. Every formula cell is indexed, including `[]` for formulas with no statically detected reads, so audit consumers can reject unsupported negative-lineage claims around runtime-addressed formulas. Written **one edge per line** (still valid JSON) so a >512 MiB graph can be read line-by-line without exceeding Node's max string length. Consumers expand a token lazily against the cells they care about. **Removed from the default output** once the closures are baked into the named maps; re-run `ete init --emit-debug` to keep it (plus the root `model-map.json`) for offline analysis or closure recomputation. |
...
A full lineage-first init still streams the graph again during the later closure
pass. Holding that multi-GB in-memory map across manifest detection would raise
peak heap beside the 5.8M-cell ground truth, so it is intentionally not cached.
A target-aware parser sidecar or disk-backed index is the remaining scale fix.
