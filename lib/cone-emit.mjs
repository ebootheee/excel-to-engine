...
function emitCycleLoop(addrs, cellExprs, idx) {
  ...
  L.push(`      if (_bad !== null) { _nonFinite = _bad; _streak++; _prevDelta = Infinity; _maxDelta = Infinity; if (_streak >= 3) break; continue; }`);
...
/** Flatten a value map to { cell: scalar }: accepts a flat { cell: value } or a
 *  nested { cell: {value|v} } (defensive — real GT is flat, but be robust). */
function flattenValues(obj) {
  const flat = {};
  for (const [k, v] of Object.entries(obj)) flat[k] = (v && typeof v === 'object' && !Array.isArray(v)) ? (v.value ?? v.v ?? v) : v;
  return flat;
}

/** Resolve the base-case value map: caller-provided, else the GT sidecar
 *  (_ground-truth.json), which IS the base-case values for real models. Used both
 *  for boundary MEMBERSHIP (range-leaf rescue — the gtKeys landmine, ADR-026
 *  invariant #6) AND boundary base folding. FAILS LOUD when neither source exists:
 *  without base values, range-member RAW-LEAF boundary cells would be silently
 *  dropped and read 0 at runtime — a silent correctness loss (QA finding). */
function resolveBaseValues(chunkedDir, baseValues) {
  if (baseValues) return flattenValues(baseValues);
  const gtPath = join(chunkedDir, '_ground-truth.json');
  if (existsSync(gtPath)) return flattenValues(JSON.parse(readFileSync(gtPath, 'utf-8')));
  throw new Error(
    `cone-emit: no base-case values for ${chunkedDir} — pass opts.baseValues or ensure ${gtPath} exists. ` +
    `A cone needs base values to fold boundary constants AND to rescue range-member leaf cells ` +
    `(ADR-026 invariant #6); without them the cone would silently read 0 for those cells.`,
  );
}
...
export function buildConesFromManifest(chunkedDir, { write = true, modelHash, baseValues } = {}) {
  ...
  // A named cell ref may be a string OR an aggregate { cells:[...], op } (multi-class
  // carry / multi-facility debt — manifest.mjs resolveSiblingAggregate). Flatten to
  // component cells: passing the aggregate OBJECT to buildScopePlan would id.get(obj)
  // → undefined → the component cells never enter the cone (the "multi-class
  // understatement" failure class in CLAUDE.md). QA finding — must-fix.
  const cellsOf = (cf) => (typeof cf === 'string' ? [cf] : (cf && Array.isArray(cf.cells) ? cf.cells : []));
