...
import { emitAuditLineage } from './audit-lineage.mjs';
 ...
 */
// Ratio/coverage schedule types whose labels embed flow/balance words
// ("DSCR (NOI / Debt Service)", "Debt Yield (NOI / Debt)") and so would be
// grabbed by a flow/balance key's loose label regex. A flow key (freeCashFlow)
// or balance key (outstandingDebt) must only bind these via an EXACT type match,
// never by label — otherwise freeCashFlow mis-binds the debt-yield row.
const RATIO_SCHEDULE_TYPES = new Set(['coverage', 'debt_yield', 'ltv']);
...
function findScheduleRow(manifest, gt, type, regex, fallbackFn = null) {
  ...
  if (manifest.schedules) {
    for (const s of manifest.schedules) {
      // A ratio schedule is only returned to the key that explicitly asked for
      // its type; the loose label-regex path skips it so a flow/balance key
      // can't steal a DSCR/debt-yield/LTV row whose label contains "NOI"/"Debt".
      if (s.type === type) return { sheet: s.sheet, row: s.row, label: s.label };
      if (regex.test(s.label) && !RATIO_SCHEDULE_TYPES.has(s.type)) {
...
export function enumerateOutputCells(manifest, gt = null) {
  ...
  if (manifest.equity?.classes) {
    ...
    for (const ec of manifest.equity.classes) {
      ...
      // Issue #25: value-bearing per-class cells — the MIP/promote proceeds, the
      // class valuation, and the hurdle/pref cell — pinned WHEN the manifest
      // carries them (set by a detector or `ete manifest set`), not just MOIC/IRR.
      // inferFormat already maps proceeds/valuation -> currency and hurdle ->
      // fraction, so these need no extra wiring downstream.
      if (ec.proceeds) out[`${prefix}proceeds`] = ec.proceeds;
      if (ec.valuation) out[`${prefix}valuation`] = ec.valuation;
      if (ec.hurdle) out[`${prefix}hurdle`] = ec.hurdle;
    }
  }
  ...
  if (manifest.timeline?.columnMap) {
    ...
    if (cols.length > 0) {
      ...
      // 2. outstandingDebt — broadened to bind a roll-forward "Facility/Closing/
      // Ending Balance" amortization row (the lender/CFO centerpiece), not just
      // an "Outstanding Debt" label. The manifest.schedules type match (step 1 of
      // findScheduleRow) already routes a detected debt_balance schedule here; the
      // regex below is the dynamic-search fallback when no schedule was detected.
      addScheduleKey('outstandingDebt', 'debt_balance', /debt.*balance|loan.*balance|principal.*balance|outstanding.*debt|facility.*balance|(?:debt|loan|facility|senior|mezz|note).*(?:closing|ending).*balance/i, (m, g) => {
      ...
      // 5. Per-year credit/coverage series — the dials a lender/CFO actually
      // reads across the hold (DSCR, debt yield, LTV). Emitted as schedules with
      // a correctly-formatted terminal scalar (multiple/fraction), never summed.
      // Keyed distinctly from the covenant SCALARS (dscr/ltv) so both coexist:
      // the scalar answers "what is it at exit?", the series answers "how does it
      // trend?". A model without these rows simply emits nothing (addScheduleKey
      // is conditional).
      addScheduleKey('dscrByYear', 'coverage', /\bdscr\b|debt.*service.*coverage|coverage.*ratio/i);
      addScheduleKey('debtYieldByYear', 'debt_yield', /debt.*yield/i);
      addScheduleKey('ltvByYear', 'ltv', /\bltv\b|loan.?to.?value/i);

      // 6. Per-class distributions (split by class)
      if (manifest.equity?.classes) {
        for (const ec of manifest.equity.classes) {
...
function inferFormat(name, value) {
  ...
  // Dollar figures.
  if (/ebitda|terminal|valuation|value|carry|basis|gain|cash|debt|price|proceeds|revenue|equity|capital|noi|distribut|\bfund|paid|\bnav\b|commit|contribut|principal|\barr\b|income|sde|size/.test(n)) return 'currency';
  return 'number';
}
...
export function collectNamedOutputs(manifest, gt, namedRangeMap = new Map()) {
  ...
  for (const [name, manifestRef] of Object.entries(cells)) {
    ...
    if (typeof manifestRef === 'string' && manifestRef.includes(':')) {
      ...
      const perYear = [];
      let lastAddr = null; // address of the last POPULATED cell in the range
      ...
      // The scalar baseCaseValue must reflect what the series MEANS:
      //   - a BALANCE (debt outstanding, equity base/NAV) is a point-in-time
      //     level — the cross-year sum double-counts; the right scalar is the
      //     terminal (exit-year) level.
      //   - a RATIO/PERCENT/MULTIPLE series (debt yield, DSCR, LTV) CANNOT be
      //     summed: Σ(per-year debt yield) is a meaningless number that renders
      //     as a fabricated "$1". Its scalar is the terminal (exit) value too.
      //   - a FLOW (distributions, cash flow, NOI) genuinely accumulates → sum.
      // perYear[] is always authoritative. We detect the series' unit from its
      // label+name+first value (a fraction in (-1,1) is overwhelmingly a rate),
      // so the scalar AND its display format follow the concept, not a guess.
      // Pass the HUMANIZED name (camelCase → spaced) so inferFormat's \b-anchored
      // ratio tokens fire on a synthesized key like "ltvByYear" (\bltv\b fails on
      // "ltvbyyear"); humanize('ltvByYear') = 'Ltv By Year' matches. label is
      // still the primary signal; the value-aware net is the final backstop.
      const fmt = inferFormat(`${label} ${humanize(name)}`, perYear.length ? perYear[0].value : undefined);
      const isRatio = fmt === 'fraction' || fmt === 'percent' || fmt === 'multiple';
      const pointInTime = isBalanceSchedule(name) || isRatio;
      const lastPoint = perYear.length ? perYear[perYear.length - 1] : null;
      const baseCaseValue = perYear.length === 0
        ? null
        : (pointInTime ? lastPoint.value : perYear.reduce((acc, p) => acc + p.value, 0));
      ...
      result[name] = {
        ...
        label,
        type: 'schedule',
        // Format off the series' detected unit, NEVER a hardcoded "currency" — a
        // debt-yield/DSCR/LTV series must render "%"/"x", not "$".
        format: fmt,
        // 'sum' (flows accumulate) vs 'terminal' (balances + ratios are
        // point-in-time); tells a consumer how the scalar derives from perYear[].
        aggregation: pointInTime ? 'terminal' : 'sum',
        baseCaseValue,
        // A point-in-time scalar (balance or ratio) IS a real engine cell — the
        // last populated cell of the range (the terminal level). Exposing it as
        // `cell` lets a consumer's per-cell drift check (verify-engine,
        // example.mjs) resolve a schedule to a value instead of reading
        // values[undefined] and false-flagging "contract may be stale". A FLOW's
        // scalar is a cross-year sum that equals no single cell, so it gets no
        // `cell` (consumers aggregate cellRange/perYear for those).
        ...(pointInTime && lastAddr ? { cell: lastAddr } : {}),
        ...(pointInTime && lastPoint ? { terminalYear: lastPoint.year } : {}),
        source,
      };
      continue;
    }
    ...
    // A cap-rate / exit-yield output is a RATE (e.g. 8.5%), never an x-multiple —
    // even though its generic manifest key is "exitMultiple". Format off the
    // detected CONCEPT so the contract + INTEGRATION render "8.5%", not "0.09×"
    // (a yield shown as a multiple). Mirrors the summary lens, which already
    // prints these via fmtPct.
    let fmt = inferFormat(name, baseCaseValue);
    if (manifest.outputs?.exitMultiple?.type === 'cap_rate_inverse'
        && (name === 'exitMultiple' || name.endsWith('.exitMultiple'))) {
      fmt = 'fraction';
    }
    const entry = {
      cell,
...
export function collectNamedInputs(workbook, manifest = null, gt = null) {
  ...
  if (manifest && gt) {
    // 1. exitMultiple — but NEVER for a cap_rate_inverse concept. When the exit
    // metric is a yield/cap rate (e.g. an infra exit yield, a debt yield), its
    // cell is an OUTPUT the model computes, not an exit-MULTIPLE the user dials.
    // Minting a phantom "exitMultiple" driver on a yield cell wires a yield as a
    // what-if lever (and the real defined-name lever, e.g. ExitYield, already
    // exposes it correctly). Skip it so the explorer never offers "exit multiple
    // = 0.085".
...
/**
 * Collapse named-input levers that point at the SAME cell into one canonical
 * entry. A manifest-driver (`exitMultiple`/`exitYearSelector`/`hurdleRate`) and a
 * workbook defined name (`ExitCapRate`, `ExitMultiple`, `ExitYield`) frequently
 * resolve to the same input cell. collectNamedInputs guards driver insertion by
 * KEY NAME (`!result.exitMultiple`), not by cell, so both land — and because the
 * dependency closure is computed per input NAME, the human-named one often shows
 * `affectsOutputs: []` while the cryptic driver twin holds the real wiring. The
 * analyst then sees their actual slider ("Exit Cap Rate") looking inert.
 *
 * Run this AFTER closures are baked so the union reflects real affectsOutputs.
 * Keep the defined-name identity (the name/excelName the analyst recognizes), but
 * carry the UNION of affectsOutputs/dependsOnNamedInputs and the richer metadata
 * (format/min/max/step) from whichever twin had it. Mutates + returns the map.
 *
 * @param {Object} namedInputs - name → entry (post-closure)
 * @returns {{ map: Object, merged: number }}
 */
export function dedupeInputsByCell(namedInputs) {
  if (!namedInputs) return { map: namedInputs, merged: 0 };
  const firstForCell = new Map(); // cell → kept key
  const out = {};
  let merged = 0;
  for (const [name, entry] of Object.entries(namedInputs)) {
    const cell = entry.cell;
    if (!cell || !firstForCell.has(cell)) {
      if (cell) firstForCell.set(cell, name);
      out[name] = entry;
      continue;
    }
    // Duplicate cell: merge into the kept entry.
    merged++;
    const keptKey = firstForCell.get(cell);
    const kept = out[keptKey];
    const unionAff = [...new Set([...(kept.affectsOutputs || []), ...(entry.affectsOutputs || [])])].sort();
    // Prefer the defined-name identity (human name + excelName); a manifest-driver
    // key like "exitMultiple" is generated, not what the model owner named the cell.
    const definedIsKept = kept.source === 'defined-name';
    const definedIsNew = entry.source === 'defined-name';
    let canonicalKey = keptKey;
    let canonical = kept;
    if (definedIsNew && !definedIsKept) {
      // Rekey to the defined name, but keep the driver's richer fields as a base.
      canonicalKey = name;
      canonical = { ...kept, ...entry };
      delete out[keptKey];
    } else {
      canonical = { ...entry, ...kept }; // kept identity wins; fill gaps from twin
    }
    canonical.affectsOutputs = unionAff;
    canonical.excelName = (definedIsKept ? kept.excelName : entry.excelName) || kept.excelName || entry.excelName;
    canonical.format = canonical.format || kept.format || entry.format;
    if (canonical.min == null && (kept.min != null || entry.min != null)) {
      const src = kept.min != null ? kept : entry;
      canonical.min = src.min; canonical.max = src.max; if (src.step != null) canonical.step = src.step;
    }
    // Record the merge so it's auditable (which generated twin folded in).
    canonical.mergedFrom = [...(canonical.mergedFrom || []), definedIsNew && !definedIsKept ? keptKey : name];
    canonical.source = 'defined-name'; // a recognized lever, now correctly wired
    out[canonicalKey] = canonical;
    firstForCell.set(cell, canonicalKey);
  }
  // Replace the original object's contents in place (callers hold the reference).
  for (const k of Object.keys(namedInputs)) delete namedInputs[k];
  for (const [k, v] of Object.entries(out)) namedInputs[k] = v;
  return { map: namedInputs, merged };
}
 ...
 * `"key":[...]` line on its own. The loader verifies generated graphs rather
 * than treating a readable prefix as complete: malformed edge lines, duplicate
 * keys, missing stream headers/footers, and declared edge-count mismatches all
 * throw. Legacy JSON graphs without format/count metadata remain readable for
 * closure compatibility, but their `integrity.verifiedComplete` flag is false
 * and audit lineage will not certify absence from them.
 *
 * @param {string} graphPath
 * @param {number} [streamThreshold] - byte size above which to stream (testing hook)
 * @returns {{edges:Object<string,string[]>, edgeCount:number, formulaIdx:Map|null, integrity:Object}}
 */
export function loadDependencyGraph(graphPath, streamThreshold = 256 * 1024 * 1024, opts = {}) {
  const buildIndex = opts.buildIndex === true;
  const graphIntegrity = (format, declaredEdgeCount, parsedEdgeCount) => ({
    format: typeof format === 'string' ? format : null,
    declaredEdgeCount: Number.isInteger(declaredEdgeCount) ? declaredEdgeCount : null,
    parsedEdgeCount,
    verifiedComplete: (format === 'cell-dependency-edges-v2' || format === 'cell-dependency-edges-v3')
      && Number.isInteger(declaredEdgeCount)
      && declaredEdgeCount === parsedEdgeCount,
    // v3 is the first graph contract that indexes every formula cell, including
    // formulas whose only reads are runtime-addressed (OFFSET/INDIRECT).
    allFormulaCells: format === 'cell-dependency-edges-v3',
  });
  const validateEdge = (key, refs) => {
    if (typeof key !== 'string' || !key.includes('!')) {
      throw new Error(`Dependency graph contains an invalid edge key: ${JSON.stringify(key)}`);
    }
    if (!Array.isArray(refs) || refs.some((ref) => typeof ref !== 'string')) {
      throw new Error(`Dependency graph edge ${key} is not an array of reference strings`);
    }
  };
  ...
  if (size < streamThreshold) {
    const doc = JSON.parse(readFileSync(graphPath, 'utf-8'));
    if (!doc || typeof doc !== 'object' || !doc.edges || typeof doc.edges !== 'object' || Array.isArray(doc.edges)) {
      throw new Error('Dependency graph is missing its edges object');
    }
    const edges = doc.edges;
    const keys = Object.keys(edges);
    for (const key of keys) validateEdge(key, edges[key]);
    if (Number.isInteger(doc.edgeCount) && doc.edgeCount !== keys.length) {
      throw new Error(`Dependency graph edgeCount mismatch: declared ${doc.edgeCount}, parsed ${keys.length}`);
    }
    return {
      edges,
      edgeCount: keys.length,
      formulaIdx: buildIndex ? buildCellIndex(keys) : null,
      integrity: graphIntegrity(doc.format, doc.edgeCount, keys.length),
    };
  }

  // Large graph: stream it. One edge per line — parse each line independently.
  const edges = Object.create(null);
  const formulaIdx = buildIndex ? new Map() : null;
  let edgeCount = 0;
  let format = null;
  let declaredEdgeCount = null;
  ...
  try {
    ...
    const flush = (rawLine) => {
      ...
      if (!headerSeen) {
        if (!line.startsWith('{') || !line.endsWith('"edges":{')) {
          throw new Error('Dependency graph stream is missing its generated header');
        }
        let header;
        try { header = JSON.parse(line + '}}'); } catch (e) {
          throw new Error(`Dependency graph header is malformed: ${e.message}`);
        }
        format = header.format || null;
        headerSeen = true;
        return;
      }
      if (footerSeen) throw new Error('Dependency graph contains data after its footer');
      if (line.charCodeAt(0) === 125 /* } */) {
        const match = /^\},"edgeCount":(\d+)\}$/.exec(line);
        if (!match) throw new Error('Dependency graph footer is malformed');
        declaredEdgeCount = Number(match[1]);
        footerSeen = true;
        return;
      }
      if (line.charCodeAt(0) !== 34 /* " */) {
        throw new Error('Dependency graph contains a malformed edge line');
      }
      const s = line.charCodeAt(line.length - 1) === 44 /* , */ ? line.slice(0, -1) : line;
      let obj;
      try { obj = JSON.parse('{' + s + '}'); } catch (e) {
        throw new Error(`Dependency graph edge line is malformed: ${e.message}`);
      }
      const entries = Object.entries(obj);
      if (entries.length !== 1) throw new Error('Dependency graph edge line must contain exactly one edge');
      const [key, refs] = entries[0];
      validateEdge(key, refs);
      if (Object.prototype.hasOwnProperty.call(edges, key)) {
        throw new Error(`Dependency graph contains duplicate edge key ${key}`);
      }
      edges[key] = refs;
      edgeCount++;
      if (formulaIdx) addCellToIndex(formulaIdx, key);
    };
  ...
  if (!headerSeen) throw new Error('Dependency graph stream is empty');
  if (!footerSeen) throw new Error('Dependency graph stream is truncated before its footer');
  if (declaredEdgeCount !== edgeCount) {
    throw new Error(`Dependency graph edgeCount mismatch: declared ${declaredEdgeCount}, parsed ${edgeCount}`);
  }
  if (formulaIdx) finalizeCellIndex(formulaIdx);
  return {
    edges,
    edgeCount,
    formulaIdx,
    integrity: graphIntegrity(format, declaredEdgeCount, edgeCount),
  };
}

/** Backward-compatible edge-only loader used by existing callers/tests. */
export function loadDependencyEdges(graphPath, streamThreshold = 256 * 1024 * 1024) {
  return loadDependencyGraph(graphPath, streamThreshold).edges;
}

/**
 * Minimal ground-truth seed for converging a cross-sheet cluster in isolation (#33).
 *
 * Seeding the whole ~5.8M-cell ground truth into every cluster child OOMs (8GB+).
 * The cluster only needs (1) its EXTERNAL reads — upstream input cells it reads
 * but does NOT compute — pinned as fixed boundary values, and (2) a warm start for
 * its own cells: those are overwritten by the convergence loop each pass, but
 * starting them at their GT value avoids a cold-start divide-by-zero -> Inf/NaN in
 * waterfall/coverage formulas (the T-076 failure mode). The rest of the model (the
 * non-cluster, non-referenced bulk) is excluded — that is the memory win.
 *
 * Range refs are expanded lazily against a GT-key index (issue #32) so a formula
 * reading a giant cross-sheet range never materializes the whole range.
 *
 * @param {string[]} clusterSheets   sheet names forming the circular cluster
 * @param {Object<string,string[]>|null} edges  forward edges cell -> [refs/ranges]
 * @param {Iterable<string>} gtKeys   ground-truth cell addresses
 * @param {{warmStartCluster?: boolean}} [opts]  warmStartCluster=false seeds only
 *        the cluster's raw-input cells (cold-starts computed cells — minimal seed)
 * @returns {Set<string>} cell addresses to seed
 */
export function computeClusterSeed(clusterSheets, edges, gtKeys, opts = {}) {
  const warmStart = opts.warmStartCluster !== false;
  const clusterSet = new Set(clusterSheets);
  const gtSet = gtKeys instanceof Set ? gtKeys : new Set(gtKeys);
  const seed = new Set();
  const ownedFormula = new Set(); // cluster cells that are formula keys (computed)

  if (edges) {
    const gtIdx = buildCellIndex(gtSet); // lazy range membership (issue #32)
    for (const k in edges) {
      const bang = k.indexOf('!');
      if (bang < 0 || !clusterSet.has(k.slice(0, bang))) continue; // only cluster-owned formula cells
      ownedFormula.add(k);
      for (const ref of edges[k]) {
        const rb = typeof ref === 'string' ? ref.indexOf('!') : -1;
        if (rb < 0) continue;
        if (clusterSet.has(ref.slice(0, rb))) continue; // internal ref — the loop computes it
        if (ref.indexOf(':') < 0) {
          if (gtSet.has(ref)) seed.add(ref);            // single external cell
        } else {
          const p = parseRefToken(ref);
          if (p) forEachCellInRange(gtIdx, p, (cell) => seed.add(cell)); // GT cells in the range
          else if (gtSet.has(ref)) seed.add(ref);
        }
      }
    }
  }

  // Cluster-resident cells. Warm-start every GT cell on a cluster sheet (computed
  // cells get overwritten by the loop; raw inputs/constants stay). With warmStart
  // off, seed only the raw inputs (cells with no outgoing formula edge).
  for (const key of gtSet) {
    const bang = key.indexOf('!');
    if (bang < 0 || !clusterSet.has(key.slice(0, bang))) continue;
    if (warmStart || !ownedFormula.has(key)) seed.add(key);
  }
...
export function buildCellIndex(cells) {
  ...
  for (const cell of cells) {
    addCellToIndex(idx, cell);
  }
  finalizeCellIndex(idx);
  return idx;
}

function addCellToIndex(idx, cell) {
...
function finalizeCellIndex(idx) {
 ...
 */
export function forEachCellInRange(idx, p, fn) {
...
export function computeOutputClosures(namedOutputs, namedInputs, edges, fallbacks = {}, opts = {}) {
  ...
  const formulaIdx = opts.formulaIdx || buildCellIndex(Object.keys(edges));
  ...
  for (const [outName, out] of Object.entries(namedOutputs)) {
    const targets = [];
    // A schedule may now carry BOTH a scalar `cell` (the terminal level) and the
    // full `cellRange`. Collect both so the dependency closure spans every year,
    // not just the terminal cell (the Set in the BFS dedupes the overlap).
  ...
  if (attachDeps) {
    for (const [name, info] of Object.entries(namedInputs)) {
      ...
      // A named input whose OWN cell is itself a formula target (a key in the
      // forward edge map) is DERIVED/backsolved — the engine recomputes it from
      // its formula, so overriding it is futile and it must never be offered as
      // a what-if dial. Flag it so the integration demo + any explorer skip it.
      // A raw assumption cell has no outgoing edges → left unflagged, so this is
      // inert for the typical defined-name lever. (No graph → field stays unset,
      // same "unknown ⇒ treat as live" convention as affectsOutputs.)
      if (info.cell && edges && edges[info.cell]) info.derived = true;
    }
  }
  return violations;
}
...
function rangeWalker(formulaIdx) {
  if (!formulaIdx) return null;
  return (rangeRef, fn) => {
    const parsed = parseRefToken(rangeRef);
    if (parsed?.isRange) forEachCellInRange(formulaIdx, parsed, fn);
  };
}

/**
 * Emit only the configured audit proof, without running manifest detectors,
 * named-input discovery, fallback scans, dependency closures, or cell typing.
 * `ete init` uses this immediately after the parser when an explicit template
 * or prior manifest already supplies owner-authored pins. That gives a large
 * model a durable proof before the unrelated O(N) manifest/contract work.
 *
 * Truthfulness is unchanged: exact formulas still come from the source
 * workbook, values from ground truth, and paths from the complete dependency
 * graph. Missing evidence produces a partial/unavailable artifact.
 */
export function emitConfiguredAuditLineage(chunkedDir, opts = {}) {
  const manifestPath = join(chunkedDir, 'manifest.json');
  const gtPath = join(chunkedDir, '_ground-truth.json');
  const manifest = opts.manifest || (existsSync(manifestPath)
    ? JSON.parse(readFileSync(manifestPath, 'utf-8'))
    : null);
  if (!manifest?.auditTraces || typeof manifest.auditTraces !== 'object') {
    return { configured: false, written: false, status: 'not-configured', traceCount: 0 };
  }

  const gt = opts.gt || (existsSync(gtPath) ? JSON.parse(readFileSync(gtPath, 'utf-8')) : {});
  let workbook = opts.workbook || null;
  if (!workbook && opts.excelPath && existsSync(opts.excelPath)) {
    try { workbook = loadWorkbook(opts.excelPath); } catch { /* disclosed as partial below */ }
  }
  // Trace-root resolution only needs the manifest's pinned output cells. Do
  // not invoke collectNamedOutputs here: its schedule fallbacks and defined-
  // name enrichment belong to the full contract pass and can scan all GT.
  const namedOutputs = {};
  for (const [name, ref] of Object.entries(enumerateOutputCells(manifest, null))) {
    if (typeof ref === 'string' && !ref.includes(':')) namedOutputs[name] = { cell: ref };
    else if (Array.isArray(ref?.cells) && ref.cells.length === 1) namedOutputs[name] = { cell: ref.cells[0] };
  }

  let graph = { edges: null, formulaIdx: null, edgeCount: 0 };
  const graphPath = join(chunkedDir, 'dependency-graph.json');
  if (existsSync(graphPath)) {
    try { graph = loadDependencyGraph(graphPath, undefined, { buildIndex: true }); } catch { /* partial */ }
  }

  return emitAuditLineage(chunkedDir, {
    manifest,
    namedOutputs,
    workbook,
    groundTruth: gt,
    edges: graph.edges,
    workbookPath: opts.excelPath || null,
    groundTruthPath: gtPath,
    graphIntegrity: graph.integrity,
    forEachFormulaCellInRange: rangeWalker(graph.formulaIdx),
  });
}
...
export function emitManifestMaps(chunkedDir, opts = {}) {
  ...
  // Named outputs are sufficient to resolve trace roots. Emit configured audit
  // lineage NOW, before the unrelated named-input, fallback, closure, and
  // cell-type work. On a 5.8M-cell model those later phases are substantial;
  // the owner-authored proof should already be durable if they are interrupted.
  ...
  if (existsSync(graphPath)) {
    try {
      const graph = loadDependencyGraph(graphPath, undefined, { buildIndex: true });
      edges = graph.edges;
      formulaIdx = graph.formulaIdx;
      graphIntegrity = graph.integrity;
  ...
  const lineage = emitAuditLineage(chunkedDir, {
    manifest,
    namedOutputs,
    workbook,
    groundTruth: gt,
    ...
    groundTruthPath: gtPath,
    graphIntegrity,
    forEachFormulaCellInRange: rangeWalker(formulaIdx),
  });
  stats.auditLineage = {
    configured: lineage.configured,
    written: lineage.written,
    status: lineage.status,
    traceCount: lineage.traceCount,
  };
  if (lineage.written) written.push('audit-lineage.json');

  // Build the remaining named map, then enrich both with dependency closures,
  // so each contract file is written once with its closures in place.
  ...
  const fallbackViolations = computeOutputClosures(
    namedOutputs, namedInputs, edges || {}, fallbacks,
    { attachDeps: edges !== null, formulaIdx },
  );
  if (edges !== null) {
    stats.dependencyEdges = dependencyEdgeCount;
    stats.closures = true;
  }
  ...
  // Collapse duplicate-cell levers (defined-name + manifest-driver on one cell)
  // AFTER closures are baked, so the surviving entry carries the real
  // affectsOutputs union — not the empty closure the human-named twin had.
  if (namedInputs) {
    const { merged } = dedupeInputsByCell(namedInputs);
    if (merged > 0) stats.inputsMerged = merged;
  }
