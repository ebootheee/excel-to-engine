...
/**
 * Synchronous variant of {@link extractCellExprs}, for sync callers like the init
 * pipeline (which already holds the model in memory). Reads each module whole;
 * fine under the init heap. Prefer the async streaming version in long-lived /
 * memory-tight contexts.
 *
 * @param {string} chunkedDir
 * @returns {Map<string,string>}
 */
export function extractCellExprsSync(chunkedDir) {
  const sheetsDir = join(chunkedDir, 'sheets');
  if (!existsSync(sheetsDir)) throw new Error(`extractCellExprsSync: no sheets/ under ${chunkedDir}`);
  const out = new Map();
  for (const f of readdirSync(sheetsDir)) {
    if (!f.endsWith('.mjs') || f.startsWith('_')) continue;
    const src = readFileSync(join(sheetsDir, f), 'utf-8');
