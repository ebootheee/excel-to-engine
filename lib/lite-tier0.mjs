...
// ---------------------------------------------------------------------------
// 3. deriveModelHash — cheap + deterministic, mirrors scope-plan's discipline.
//    sha256 over the small _graph.json bytes if present, else over a stable
//    digest of the manifest's structural cell refs. NEVER hash the 177MB GT.
// ---------------------------------------------------------------------------
...
function hashStructuralRefs(refs) {
...
export function loadTier0(paramsPath) {
  ...
  const recorded = params.provenance?.modelHash;
  const recordedStructural = params.provenance?.structuralHash;
  const embeddedRefs = params.provenance?.structuralRefs;

  // ── (A) UNCONDITIONAL self-integrity check (the PRIMARY handoff scenario) ────
  // The artifact ships WITHOUT the 564MB graph / 185MB GT. The structural-refs
  // hash is verifiable from the artifact ALONE: re-digest the embedded refs and
  // compare to the stamped structuralHash. This catches a tampered/corrupt
  // artifact even when no model files are co-located — the case where provenance
  // enforcement matters most. THROW (do not skip) if the refs are missing or
  // the digest disagrees.
  if (!Array.isArray(embeddedRefs) || typeof recordedStructural !== 'string') {
    throw new Error(
      `Tier-0 artifact ${p} cannot be provenance-verified: missing ` +
      `provenance.structuralRefs / structuralHash. Re-run emitTier0 to stamp them.`,
    );
  }
  const liveStructural = hashStructuralRefs(embeddedRefs);
  if (liveStructural !== recordedStructural) {
    throw new Error(
      `Tier-0 artifact tampered: structuralHash ${recordedStructural} != ` +
      `${liveStructural} (re-digest of embedded structuralRefs). Re-run emitTier0.`,
    );
  }

  // ── (B) Live-model check when resolvable (graph present → strongest hash) ────
  // If the recorded chunkedDir resolves AND a graph is present, re-derive the
  // graph-based modelHash and compare. If only the manifest is present, compare
  // the manifest-fallback hash to the embedded structuralHash.
  const chunkedRef = params.provenance?.chunkedDir;
  ...
  if (hasGraph) {
    ...
    if (live !== recorded) {
      throw new Error(`Tier-0 artifact stale: modelHash ${recorded} != ${live}. Re-run emitTier0.`);
    }
  } else if (hasManifest) {
    const liveManifestHash = deriveModelHash(chunkedDir); // manifest fallback
    if (liveManifestHash !== recordedStructural) {
      throw new Error(
        `Tier-0 artifact stale: manifest structural hash ${liveManifestHash} != ` +
        `recorded ${recordedStructural}. Re-run emitTier0.`,
      );
    }
  } else {
    // ── (C) SHIPPED WITHOUT THE GRAPH/MANIFEST — the primary handoff scenario ──
    // No live model to validate the graph-based modelHash against. modelHash is
    // graph-derived (≠ structuralHash) ONLY when a graph was present at emit; if
    // a graph was present at emit it should be present for the live check, so a
    // graph-derived modelHash with NO co-located graph is a stale/relocated
    // artifact we cannot verify. The only modelHash we CAN verify from the
    // artifact alone is the manifest-fallback hash (== structuralHash). Require
    // modelHash to equal it; throw otherwise rather than silently trusting a
    // value we cannot re-derive. (Self-contained, graph-free handoffs should be
    // emitted from a dir WITHOUT a graph so modelHash == structuralHash.)
    if (recorded !== recordedStructural) {
      throw new Error(
        `Tier-0 artifact ${p} ships without its _graph.json/manifest.json and its ` +
