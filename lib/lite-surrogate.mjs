...
import {
  deriveModelHash, structuralRefs,
} from './lite-tier0.mjs';
...
/** sha256 of the structural-refs digest (identical to lite-tier0's private one). */
...
export function loadSurrogate(paramsPath) {
  ...
  if (mode === 'self-contained') {
    ...
    if (!Array.isArray(embeddedRefs) || typeof recordedStructural !== 'string') {
      throw new Error(
        `Surrogate artifact ${p} cannot be provenance-verified: missing ` +
        `provenance.structuralRefs / structuralHash. Re-run emitSurrogate.`,
      );
    }
    const liveStructural = hashStructuralRefs(embeddedRefs);
    if (liveStructural !== recordedStructural) {
      throw new Error(
        `Surrogate artifact tampered: structuralHash ${recordedStructural} != ` +
        `${liveStructural} (re-digest of embedded structuralRefs). Re-run emitSurrogate.`,
      );
    }

    // ── (B) Live-model check when resolvable. ──────────────────────────────────
    const chunkedRef = prov.chunkedDir;
    let chunkedDir = chunkedRef;
    if (chunkedRef && chunkedRef.startsWith('.')) chunkedDir = join(dirname(p), chunkedRef);
    ...
    if (hasGraph) {
      const liveModel = deriveModelHash(chunkedDir);
      if (liveModel !== recorded) {
        throw new Error(`Surrogate artifact stale: modelHash ${recorded} != ${liveModel}. Re-run emitSurrogate.`);
      }
    } else if (hasManifest) {
      const liveManifestHash = deriveModelHash(chunkedDir); // manifest fallback
      if (liveManifestHash !== recordedStructural) {
        throw new Error(
          `Surrogate artifact stale: manifest structural hash ${liveManifestHash} != ` +
          `recorded ${recordedStructural}. Re-run emitSurrogate.`,
        );
      }
    } else {
      // ── (C) Graph-free handoff guard. ────────────────────────────────────────
      if (recorded !== recordedStructural) {
        throw new Error(
          `Surrogate artifact ${p} ships without its _graph.json/manifest.json and its ` +
