 ...
 */
export function buildNamedRangeMap(workbook) {
  const map = new Map();
...
export function loadWorkbook(filePath, options = {}) {
  // Read bytes via Node fs and parse from a buffer rather than XLSX.readFile:
  // the SheetJS ESM build ships without an fs binding, so readFile throws
  // "Cannot access file" under Node. XLSX.read(buffer) is environment-agnostic.
...
const LABEL_ALIASES = {
  ...
  carry: [
    'carry', 'carried interest', 'gp carry', 'total carry',
    'promote', 'gp promote', 'incentive', 'performance fee',
    'performance allocation', 'carried interest allocation',
  ],
  prefReturn: [
    'preferred return', 'pref return', 'pref', 'preferred',
    'lp preferred', 'lp pref', 'hurdle', 'hurdle rate',
    'tier 1', 'return of capital',
  ],
  catchUp: [
    'catch-up', 'catch up', 'catchup', 'gp catch-up',
    'catch-up payment', 'tier 2',
  ],
  distributions: [
    'distributions', 'total distributions', 'cumulative distributions',
    'lp distributions', 'net distributions', 'cash distributions',
    'distributed', 'amount distributed',
  ],
  peakEquity: [
    'peak equity', 'total equity', 'equity invested', 'equity basis',
    'equity drawn', 'cumulative equity', 'total invested',
    'committed equity', 'equity commitment',
  ],
  waterfallTier: [
    'tier', 'tier 1', 'tier 2', 'tier 3', 'tier 4',
    'tranche', 'waterfall tier', 'distribution tier',
  ],
  mip: [
    'mip', 'management incentive', 'ltip', 'class b',
    'phantom equity', 'promote pool', 'incentive plan',
    'management incentive plan', 'carried interest pool',
  ],
};
...
// ---------------------------------------------------------------------------
// Waterfall Structure Detection (from ground truth)
// ---------------------------------------------------------------------------

/**
 * Extract waterfall structure from a parsed model's ground truth.
 *
 * Scans for tier labels ("Tier 1", "LP Preferred Return", "Catch-Up", etc.),
 * hurdle rates, carry percentages, and cash flow series. Works with the
 * _ground-truth.json output from the Rust parser.
 *
 * @param {Object} groundTruth - { "Sheet!Cell": value } map from _ground-truth.json
 * @param {Object} [options={}]
 * @param {string[]} [options.waterfallSheets] - Sheet names to search (auto-detected if omitted)
 * @returns {Object} Waterfall metadata
 */
export function extractWaterfallStructure(groundTruth, options = {}) {
  const entries = Object.entries(groundTruth);

  // Step 1: Find waterfall/carry/promote sheets
  const sheetCounts = {};
  const waterfallKeywords = ['carry', 'promote', 'waterfall', 'distribution', 'returns', 'investor'];
  for (const [addr, val] of entries) {
    if (typeof val !== 'string') continue;
    const sheet = addr.split('!')[0];
    const lower = val.toLowerCase();
    for (const kw of waterfallKeywords) {
  ...
  const waterfallSheets = options.waterfallSheets || Object.entries(sheetCounts)
    .filter(([, count]) => count >= 3)
    .sort(([, a], [, b]) => b - a)
    .map(([name]) => name);

  if (waterfallSheets.length === 0) {
    return { found: false, sheets: [], tiers: [], cashFlows: null };
  }

  // Step 2: Extract tier structure from labels
  const tiers = [];
  const tierPatterns = [
    { pattern: /tier\s*1|lp\s*pref|preferred\s*return|return\s*of\s*capital/i, name: 'Preferred Return', type: 'pref' },
    { pattern: /tier\s*2|catch[\s-]*up/i, name: 'Catch-Up', type: 'catchup' },
    { pattern: /tier\s*3|residual|80[\s/]*20|split/i, name: 'Residual Split', type: 'residual' },
    { pattern: /tier\s*4/i, name: 'Tier 4', type: 'secondary_hurdle' },
  ];

  for (const sheet of waterfallSheets) {
    const sheetEntries = entries.filter(([k]) => k.startsWith(sheet + '!'));
    for (const [addr, val] of sheetEntries) {
      if (typeof val !== 'string') continue;
      for (const tp of tierPatterns) {
        if (tp.pattern.test(val) && !tiers.find(t => t.type === tp.type && t.sheet === sheet)) {
          // Look for a percentage in the same row or nearby cells
          const cellPart = addr.split('!')[1];
          const row = cellPart.replace(/[A-Z]+/g, '');
          const rate = findNearbyRate(sheetEntries, sheet, row);
          tiers.push({
            type: tp.type,
            name: val.trim(),
            sheet,
  ...
  const keyValues = {};
  const valuePatterns = {
    totalCarry: /total\s*(carry|promote|gp\s*carry|carried\s*interest)/i,
    prefRate: /pref.*rate|hurdle\s*rate|preferred.*return.*%/i,
    carryRate: /carry\s*%|carry\s*rate|promote\s*%|gp\s*share/i,
    equityBasis: /peak\s*equity|total\s*equity|equity\s*basis|equity\s*invested/i,
    grossMOIC: /gross\s*mo[ic]|pre[\s-]*promote\s*mo[ic]|gross\s*multiple/i,
    netMOIC: /net\s*mo[ic]|post[\s-]*promote\s*mo[ic]|net\s*multiple/i,
    grossIRR: /gross\s*irr|pre[\s-]*promote\s*irr/i,
    netIRR: /net\s*irr|post[\s-]*promote\s*irr/i,
  };
  ...
  for (const sheet of waterfallSheets) {
    ...
    for (const [addr, val] of sheetEntries) {
      if (typeof val !== 'string') continue;
      for (const [key, pattern] of Object.entries(valuePatterns)) {
        if (pattern.test(val) && !keyValues[key]) {
          const cellPart = addr.split('!')[1];
          const row = cellPart.replace(/[A-Z]+/g, '');
          const numericValue = findNearbyNumeric(sheetEntries, sheet, row);
          if (numericValue !== null) {
            keyValues[key] = { value: numericValue, label: val.trim(), sheet, row: parseInt(row) };
          }
        }
      }
    }
  ...
  // Step 4: Extract cash flow series
  const cashFlows = extractCashFlowSeries(groundTruth, { sheets: waterfallSheets });

  return {
    found: true,
    sheets: waterfallSheets,
    tiers,
    keyValues,
    cashFlows,
  };
...
/**
 * Extract cash flow time series from ground truth.
 *
 * Looks for rows labeled "Cash Flow", "Net Cash Flow", "Distributions", etc.
 * and reads the numeric values across columns (one per period).
 *
 * @param {Object} groundTruth - { "Sheet!Cell": value } map
 * @param {Object} [options={}]
 * @param {string[]} [options.sheets] - Sheets to search
 * @returns {Object|null} { label, sheet, row, values: [{ col, value }] }
 */
export function extractCashFlowSeries(groundTruth, options = {}) {
  const entries = Object.entries(groundTruth);
  const cfPatterns = [
    /total\s*cash\s*flow/i,
    /net\s*cash\s*flow/i,
    /pre[\s-]*carry\s*cash/i,
    /pre[\s-]*promote\s*cash/i,
    /lp\s*cash\s*flow/i,
    /equity\s*cash\s*flow/i,
    /^cash\s*flow/i,
    /cumulative\s*distribution/i,
    /total\s*distribution/i,
  ];

  const sheets = options.sheets || [...new Set(entries.map(([k]) => k.split('!')[0]))];
  ...
  for (const sheet of sheets) {
    ...
    for (const [addr, val] of sheetEntries) {
      if (typeof val !== 'string') continue;
      const matched = cfPatterns.some(p => p.test(val));
      if (!matched) continue;

      // Found a cash flow label — read all numeric values in this row
      const cellPart = addr.split('!')[1];
      const row = cellPart.replace(/[A-Z]+/g, '');
      const values = [];

      for (const [a2, v2] of sheetEntries) {
        if (typeof v2 !== 'number') continue;
        const cp2 = a2.split('!')[1];
        const r2 = cp2.replace(/[A-Z]+/g, '');
        if (r2 !== row) continue;
        const col = cp2.replace(/[0-9]+/g, '');
        values.push({ col, addr: a2, value: v2 });
      }

      // Sort by column and filter out likely non-period values
...
// Helper: find a percentage-like value near a label row
function findNearbyRate(sheetEntries, sheet, labelRow) {
  const row = parseInt(labelRow);
  for (const [addr, val] of sheetEntries) {
    if (typeof val !== 'number') continue;
    const cp = addr.split('!')[1];
    const r = parseInt(cp.replace(/[A-Z]+/g, ''));
    // Check same row and row+1
    if (r === row || r === row + 1) {
      if (val > 0 && val <= 1) return val; // Looks like a percentage
      if (val > 1 && val <= 100) return val / 100; // Stored as whole number
    }
  }
  return null;
}

// Helper: find the first numeric value to the right of a label
function findNearbyNumeric(sheetEntries, sheet, labelRow) {
  const row = labelRow;
  const candidates = [];
  for (const [addr, val] of sheetEntries) {
    if (typeof val !== 'number' || val === 0) continue;
    const cp = addr.split('!')[1];
    const r = cp.replace(/[A-Z]+/g, '');
    if (r === row) {
      const col = cp.replace(/[0-9]+/g, '');
      candidates.push({ col, value: val });
    }
  }
  candidates.sort((a, b) => a.col.localeCompare(b.col));
  // Return first non-trivial numeric value (skip column A/B which are usually labels)
  for (const c of candidates) {
    if (c.col > 'B') return c.value;
  }
