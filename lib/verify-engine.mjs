...
export async function verifyEngine(chunkedDir, opts = {}) {
  ...
  for (const name of names) {
    const o = outMap[name];
    // Skip time-series / range outputs (type "schedule", or no scalar cell):
    // their scalar baseCaseValue is a sum/terminal aggregation of a cell RANGE,
    // not a single engine cell value, so a per-cell lookup can't verify them.
    if (o.type === 'schedule' || typeof o.cell !== 'string' || o.cell.includes(':')) continue;
