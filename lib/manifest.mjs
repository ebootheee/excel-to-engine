...
export function resolveBaseCaseOutputs(manifest, gt) {
  ...
  if (manifest.equity?.classes) {
    for (const ec of manifest.equity.classes) {
      ...
      // Issue #25: value-bearing per-class cells — mirror enumerateOutputCells so
      // the CLI base-case / scenario layer stays in sync with named-outputs.json.
      if (ec.proceeds) outputs[`${prefix}proceeds`] = resolveCell(gt, ec.proceeds);
      if (ec.valuation) outputs[`${prefix}valuation`] = resolveCell(gt, ec.valuation);
      if (ec.hurdle) outputs[`${prefix}hurdle`] = resolveCell(gt, ec.hurdle);
    }
  }
...
export function generateManifest(gt, options = {}) {
  ...
  const fundLevel = detectFundLevelMetrics(gt, sheets, timeline);
  ...
  // ADR-027 Tier-0: derive the closed-form layout (per-tier GP-CF value cells +
  // pre-carry cashflow / cumulative-equity rows) the lite Tier-0 emitter needs.
  // FAIL-SOFT: only set when the per-tier GP cells reconcile to the carry total —
  // emitTier0 then derives the layout from here instead of a hardcoded fixture.
  const tier0Layout = detectTier0Layout(gt, carry);
  if (tier0Layout) carry.tier0Layout = tier0Layout;
...
/**
 * Resolve the fund's headline NAV ("residualValue") robustly.
 *
 * First-match-wins on /residual.*nav/ is wrong for FoF models: that token also
 * appears in a terminal cashflow row label (e.g. "Net CF to LP (incl. residual
 * NAV)") whose rightmost timeline column folds the full NAV harvest into a
 * year-N cash flow — overstating NAV (the bug: Cash Flows!K7 $184.8M instead of
 * the scalar Net Asset Value $161M, +14.8%). Collect every NAV/residual
 * candidate and pick the best by:
 *   1. RVPI reconciliation (decisive when available): NAV = RVPI x paid-in. A
 *      candidate within 2% of rvpi*paidIn is the fund's true NAV.
 *   2. Scalar over series: a cashflow row spreads numerics across >=3 timeline
 *      columns; the NAV is a single scalar on its row (<3 timeline numerics).
 *   3. An explicit "Net Asset Value"/"NAV" label over a generic "residual" row.
 *
 * rvpiAddr / paidInAddr are cell addresses already resolved by the caller; their
 * values are read from gt. Returns the chosen cell address, or null.
 */
function pickFundNav(gt, timeline, rvpiAddr, paidInAddr, fundShaped) {
  const navRe = /net asset value|\bnav\b/i;
  const residRe = /residual.*value|residual.*nav|unrealiz(ed|ed value)/i;
  const cols = timeline?.columnMap ? Object.keys(timeline.columnMap) : [];
  let candidates = [];
  for (const [addr, val] of Object.entries(gt)) {
    if (typeof val !== 'string') continue;
    const lower = val.toLowerCase();
    const isNav = navRe.test(lower);
    const isResid = residRe.test(lower);
    // The original detector matched only residRe. The broadened scalar-NAV match
    // (navRe) is gated on a fund-shaped model so a stray "NAV" row on a non-fund
    // model can never bind residualValue (review finding).
    if (!isResid && !(isNav && fundShaped)) continue;
    const bang = addr.lastIndexOf('!');
    const sheet = addr.substring(0, bang);
    const cm = addr.substring(bang + 1).match(/^([A-Z]+)(\d+)$/);
    if (!cm) continue;
    const labelCol = cm[1];
    const row = parseInt(cm[2], 10);
    // Anchor to the numeric cell ADJACENT to the label (as every other dollar
    // caller does), NOT the rightmost in-range value — a trailing column (% of
    // committed, QoQ change, # of funds) must not hijack the NAV cell, which
    // would also corrupt the RVPI reconciliation below (review finding).
    const best = pickRightmostInRange(gt, sheet, row, 'residualValue', { labelCol });
    if (!best) continue;
    // Timeline-column numerics on this row: many => a cashflow series; ~1 => scalar.
    let tlCount = 0;
    for (const c of cols) {
      if (typeof gt[`${sheet}!${c}${row}`] === 'number') tlCount++;
    }
    candidates.push({ addr: best.addr, value: best.value, isNav, tlCount });
  }
  if (candidates.length === 0) return null;
  if (candidates.length === 1) return candidates[0].addr;

  // Scalar-over-series is DOMINANT, not just additive: a NAV is a single scalar on
  // its row; a cashflow row spreads numerics across the timeline. When a timeline
  // exists and any scalar candidate is present, drop the series rows entirely so a
  // coincidentally-reconciling terminal cashflow can't outscore the true scalar
  // (review finding). Reconciliation then only breaks ties AMONG scalars.
  if (cols.length >= 3 && candidates.some(c => c.tlCount < 3)) {
    candidates = candidates.filter(c => c.tlCount < 3);
  }
  if (candidates.length === 1) return candidates[0].addr;

  const rvpiVal = typeof gt[rvpiAddr] === 'number' ? gt[rvpiAddr] : null;
  const paidInVal = typeof gt[paidInAddr] === 'number' ? gt[paidInAddr] : null;
  const expected = (rvpiVal != null && paidInVal != null) ? rvpiVal * paidInVal : null;

  const score = (c) => {
    let s = 0;
    if (c.isNav) s += 10;                                  // explicit Net Asset Value / NAV label
    if (expected != null && expected > 0) {
      const rel = Math.abs(c.value - expected) / expected;
      s += rel <= 0.02 ? 1000 : Math.max(0, 50 - rel * 100); // RVPI reconciliation
    }
    return s;
  };
...
/**
 * Issue #25 — value-bearing per-class cells (MIP/promote proceeds, valuation,
 * hurdle). These are NOT a single output cell on a typical MIP model: the
 * management-incentive-plan participation is modeled across PER-BLOCK cells
 * (one block per exit scenario / tranche), each with its own "MIP Proceeds" /
 * "MIP Cash Flow" / "MIP Platform Cash Flow" label. There is therefore no lone
 * "total MIP proceeds" cell to pin — the class's economic proceeds is the SUM of
 * the per-block proceeds for that class. We locate every per-block proceeds label
 * on the class's sheet and, when there is more than one, return an aggregate
 * `{ cells, op: 'sum' }` ref (the manifest + resolveCell already sum these). A
 * single block returns a plain string ref.
 *
 * FAIL-SOFT by construction: each field is only returned when its labels are
 * found AND resolve to a plausible in-range numeric. A class with no MIP labels
 * gets `{}`, so the existing `if (ec.proceeds)` guards downstream simply skip it
 * — we never emit a wrong cell.
 *
 * @param {Object} gt - ground truth
 * @param {string} sheet - the equity class's sheet (from its basisCell)
 * @param {number} anchorRow - the class's basis row (used only as a soft anchor)
 * @returns {{ proceeds?: (string|{cells:string[],op:'sum'}), valuation?: string, hurdle?: string }}
 */
function detectMipValueCells(gt, sheet, anchorRow) {
  const out = {};
  const prefix = sheet + '!';

  // A label is a string cell in the typical label columns (A/B) on this sheet;
  // MIP block templates put the row label in A or B and the value to the right.
  const labelRowsMatching = (regex) => {
    const rows = [];
    for (const [addr, val] of Object.entries(gt)) {
      if (typeof val !== 'string') continue;
      if (!addr.startsWith(prefix)) continue;
      ...
      if (!m) continue;
      const col = m[1];
      if (col !== 'A' && col !== 'B') continue; // label column only
      if (!regex.test(val)) continue;
      rows.push({ row: parseInt(m[2], 10), labelCol: col, labelText: val.trim() });
    }
    // Deterministic order so an aggregate's `cells` array is stable across runs.
    rows.sort((a, b) => a.row - b.row);
    return rows;
  };

  // ── proceeds: per-block "MIP Proceeds" / "MIP Cash Flow" / "MIP Platform
  // Cash Flow" / "Promote Proceeds". Exclude SHARE counts ("MIP Shares Issued")
  // and per-unit-price rows — those are not dollar proceeds. ──
  const PROCEEDS_RE = /(mip|promote|incentive).*(proceeds|cash\s*flow|distribution|payout)/i;
  const PROCEEDS_REJECT = /shares?|units?|per\s*(share|unit)|\bprice\b|\b%|percent/i;
  const proceedsRows = labelRowsMatching(PROCEEDS_RE).filter(r => !PROCEEDS_REJECT.test(r.labelText));
  if (proceedsRows.length > 0) {
    const cells = [];
    for (const r of proceedsRows) {
      // Pin the dollar value adjacent to the label. A MIP proceeds figure can be
      // small for an early block but is still a dollar amount, so use a broad
      // dollar range (>= $1k) rather than a tight band, anchored to the label col
      // so a trailing %-of-deal column can't hijack it.
      const best = pickRightmostInRange(gt, sheet, r.row, 'carryTotal', { labelCol: r.labelCol });
      if (best && Math.abs(best.value) >= 1000) cells.push(best.addr);
    }
    if (cells.length === 1) out.proceeds = cells[0];
    else if (cells.length > 1) out.proceeds = { cells, op: 'sum' };
  }

  // ── valuation: the share/valuation basis used for per-unit value. A scalar. ──
  const VALUATION_RE = /(mip|promote|platform|equity).*valuation|valuation.*(mip|promote|platform)|share.*value|value.*per.*(share|unit)/i;
  const valRows = labelRowsMatching(VALUATION_RE);
  for (const r of valRows) {
    const best = pickRightmostInRange(gt, sheet, r.row, 'basisCell', { labelCol: r.labelCol });
    if (best) { out.valuation = best.addr; break; } // first plausible scalar wins
  }

  // ── hurdle: the MIP/promote participation threshold — usually a MOIC multiple
  // (e.g. 1.40×) the proceeds vest above, sometimes a pref rate. Reuse the field
  // ranges: a multiple (grossMOIC range 0.1–20×) or a pref rate (0–50%). ──
  const HURDLE_RE = /(mip|promote|incentive|participation).*(hurdle|threshold|strike|vest)|(hurdle|threshold|strike).*(moic|multiple|return)/i;
  const hurdleRows = labelRowsMatching(HURDLE_RE);
  for (const r of hurdleRows) {
    // Try a MOIC-style multiple first, then a pref rate — whichever resolves.
    const best = pickRightmostInRange(gt, sheet, r.row, 'grossMOIC', { labelCol: r.labelCol })
              || pickRightmostInRange(gt, sheet, r.row, 'prefReturn', { labelCol: r.labelCol });
    if (best) { out.hurdle = best.addr; break; }
  }

  return out;
}

export function detectEquity(gt, sheets) {
  const classes = [];
  ...
  // Issue #25: value-bearing per-class cells (MIP/promote proceeds, valuation,
  // hurdle). Scanned on the class's OWN sheet (from basisCell) so each class
  // aggregates only its blocks. Fail-soft: detectMipValueCells returns {} when
  // the labels aren't found, leaving the fields unset (the downstream
  // `if (ec.proceeds)` guards then skip them — never a wrong cell).
  for (const ec of classes) {
    if (!ec.basisCell || typeof ec.basisCell !== 'string') continue;
    const bang = ec.basisCell.lastIndexOf('!');
    const sheet = ec.basisCell.substring(0, bang);
    const rowM = ec.basisCell.substring(bang + 1).match(/(\d+)$/);
    if (!rowM) continue;
    const mip = detectMipValueCells(gt, sheet, parseInt(rowM[1], 10));
    if (mip.proceeds && !ec.proceeds) ec.proceeds = mip.proceeds;
    if (mip.valuation && !ec.valuation) ec.valuation = mip.valuation;
    if (mip.hurdle && !ec.hurdle) ec.hurdle = mip.hurdle;
  }
 ...
 */
function detectFundLevelMetrics(gt, sheets, timeline) {
  const fund = {};

  // NOTE: residualValue (the FoF headline NAV) is deliberately NOT in this
  // first-match-wins list — it is resolved by pickFundNav after the loop. The
  // loose "residual NAV" token also matches a terminal cashflow row, which
  // first-match-wins would bind to a year-N cash flow (overstating NAV).
  const patterns = [
  ...
  // residualValue — the FoF headline NAV — is resolved separately (not in the
  // first-match loop above): the loose "residual NAV" token also matches a
  // terminal cashflow row, whose rightmost timeline column folds the NAV harvest
  // into a year-N cash flow and overstates NAV. Prefer the scalar Net Asset Value
  // cell, reconciled against RVPI x paid-in. See pickFundNav.
  const _nav = pickFundNav(gt, timeline, fund.rvpi, fund.paidIn, !!(fund.tvpi || fund.dpi || fund.rvpi));
  if (_nav) fund.residualValue = _nav;
...
/**
 * ADR-027 Tier-0 layout detection. The closed-form Tier-0 emitter (lib/lite-tier0)
 * needs more than the tier label/hurdle cells `detectCarryTiers` records: it needs
 * the per-tier GP-CASHFLOW *value* cells whose SUM is the carry total, plus the
 * pre-carry cashflow + cumulative-equity rows that source the promote-pool
 * denominator. This was hardcoded to a single fixture (GPP-Promote D128/D155/…);
 * here we DERIVE it from ground truth so any model whose carry sheet carries the
 * same decomposition shape can emit Tier-0.
 *
 * Strategy (anchored on the already-chosen carry.totalCell, so a sheet that stacks
 * many scenario blocks resolves to the RIGHT block):
 *   1. The carry sheet + value column + row come from carry.totalCell (the block's
 *      "Total Carried Interest" cell). The decomposition lives in the SAME value
 *      column, in the rows of that block ABOVE the total.
 *   2. Per-tier GP cashflow rows = label rows (col A/B) matching the GP-side
 *      cashflow / distribution pattern ("Tier N GP CF", "Tier N GPP Distributions",
 *      "GP Catch-Up", …) within the block, paired with the numeric in the value
 *      column on that row.
 *   3. Pre-carry cashflow row = "Total Cash Flows (pre-carry)"; cumulative-equity
 *      row = "Cumulative Equity Drawn" — both within the block.
 *
 * FAIL-SOFT / FAIL-LOUD-LATER: the layout is returned ONLY when the per-tier GP
 * cells are found AND their sum reconciles to the carry total within `tol` (the
 * model's own decomposition invariant — the genuine confidence signal, not a
 * label-match guess) AND both cashflow rows are present. Otherwise we return null
 * and emitTier0 throws a clear "layout not in manifest" error rather than
 * mis-targeting. We never emit a half-populated layout.
 *
 * @param {Object} gt - ground truth { "Sheet!A1": value }
 * @param {Object} carry - the carry block (must have totalCell already chosen)
 * @param {Object} [opts]
 * @param {number} [opts.tol=1e-6] - relative tolerance for the sum-reconciliation gate
 * @returns {Object|null} { sheet, valueCol, totalRow, tierGpCfCells, cfRow, cumEquityRow, reconRelErr } or null
 */
export function detectTier0Layout(gt, carry, opts = {}) {
  const tol = opts.tol ?? 1e-6;
  if (!carry?.totalCell) return null;

  // totalCell may be a string or an aggregate; Tier-0's closed form is single-sheet,
  // so only a plain string ref has a derivable in-sheet decomposition block.
  const total = typeof carry.totalCell === 'string' ? carry.totalCell : null;
  if (!total) return null;
  const bang = total.lastIndexOf('!');
  if (bang < 0) return null;
  const tm = total.substring(bang + 1).match(/^([A-Z]+)(\d+)$/);
  if (!tm) return null;
  const sheet = total.substring(0, bang);
  const valueCol = tm[1];
  const totalRow = parseInt(tm[2], 10);
  const totalVal = gt[total];
  if (typeof totalVal !== 'number' || totalVal === 0) return null;

  const prefix = sheet + '!';

  // Collect this sheet's label rows (col A/B) with their text, sorted by row.
  const labels = [];
  for (const [addr, val] of Object.entries(gt)) {
    if (typeof val !== 'string') continue;
    if (!addr.startsWith(prefix)) continue;
    const m = addr.substring(prefix.length).match(/^([A-Z]+)(\d+)$/);
    if (!m || (m[1] !== 'A' && m[1] !== 'B')) continue;
    labels.push({ row: parseInt(m[2], 10), col: m[1], text: val.trim() });
  }
  labels.sort((a, b) => a.row - b.row);

  // BLOCK BOUNDS: the carry sheet may stack repeated scenario blocks (each ending
  // in its own "Total Carried Interest"). Restrict to the block ENDING at totalRow:
  // its start is the row just after the previous "Total Carried Interest".
  const TOTAL_CARRY_RE = /total\s+carried\s+interest|total\s+(carry|promote|promot)/i;
  const PRECARRY_CF_RE = /total\s+cash\s*flows?\s*\(?\s*pre.?(carry|promot)/i;
  const CUM_EQUITY_RE = /cumulative\s+equity\s+drawn|cumulative\s+(capital|equity)\s+(drawn|called|contributed)/i;
  // GP-side per-tier cashflow / distribution rows (the decomposition that sums to
  // the carry total). Must be a "Tier N" row that EXPLICITLY names a GP/GPP
  // cashflow or distribution — NOT the bare tier HEADERS ("Tier 2 - Catch Up",
  // "Tier 3: Carried Interest …") that carry the rate assumptions (0.5 / 0.2), and
  // NOT the LP-side "Tier N LP CF" / "LP Distributions" rows beside them. Requiring
  // a `GP`/`GPP` token next to a CF/distribution word is the discriminator.
  const GP_TIER_RE = /tier\s*\d.*\bgpp?\b.*(cf\b|cash\s*flow|distrib|promote|payment)|tier\s*\d.*(cf\b|cash\s*flow|distrib).*\bgpp?\b/i;
  // Dollar-magnitude gate: a real GP-CF decomposition entry is a dollar amount, so
  // a rate cell (e.g. a 0.5 catch-up assumption that slipped a header match) is
  // rejected. Pref-tier GP CF is legitimately 0, so 0 is allowed (it contributes
  // nothing to the sum and is part of the disclosed tier set).
  const isGpDollar = (v) => v === 0 || Math.abs(v) >= 1000;

  let blockStart = 1;
  for (const l of labels) {
    if (l.row >= totalRow) break;
    if (TOTAL_CARRY_RE.test(l.text)) blockStart = l.row + 1; // previous block's total
  }

  // Pre-carry cashflow + cumulative-equity rows inside (blockStart..totalRow).
  let cfRow = null;
  let cumEquityRow = null;
  for (const l of labels) {
    if (l.row < blockStart || l.row >= totalRow) continue;
    if (cfRow == null && PRECARRY_CF_RE.test(l.text)) cfRow = l.row;
    if (cumEquityRow == null && CUM_EQUITY_RE.test(l.text)) cumEquityRow = l.row;
  }

  // Per-tier GP cashflow value cells in this block, in row order, paired with the
  // numeric in the SAME value column as the total. A row with no in-column numeric
  // (or a non-number) is skipped — we only keep real decomposition cells. The
  // GP_TIER_RE already excludes the LP-side rows ("Tier N LP CF" / "LP
  // Distributions"), which would otherwise break the sum reconciliation below.
  const tierGpCfCells = [];
  for (const l of labels) {
    if (l.row < blockStart || l.row >= totalRow) continue;
    if (!GP_TIER_RE.test(l.text)) continue;
    const ref = `${sheet}!${valueCol}${l.row}`;
    const v = gt[ref];
    if (typeof v !== 'number' || !isGpDollar(v)) continue;
    tierGpCfCells.push(ref);
  }

  if (tierGpCfCells.length < 2 || cfRow == null || cumEquityRow == null) return null;

  // RECONCILIATION GATE — the genuine confidence signal. The per-tier GP cells are
  // the model's OWN decomposition of the carry total, so their sum must equal the
  // total. If it doesn't reconcile, we picked the wrong rows: bail (fail-soft).
  const sum = tierGpCfCells.reduce((s, c) => s + gt[c], 0);
  const reconRelErr = Math.abs((sum - totalVal) / totalVal);
  if (reconRelErr > tol) return null;
  ...
  return {
    ...
    cumEquityRow,
    reconRelErr,
    // record the gate so a downstream reviewer can see WHY this was trusted.
    reconciledToTotal: total,
  };
}
