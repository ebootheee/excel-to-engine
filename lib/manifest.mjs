...
export const MODEL_TYPES = [
  'pe_fund', 'pe_platform', 're_fund', 'saas', 'venture_portfolio', 'three_statement',
...
export function detectModelType(gt) {
  ...
  // Real-estate DEBT (lender-side RE: NOI present, but debt-yield/DSCR/covenant
  // headline). Must outscore plain re_fund, which also matches NOI/cap-rate.
  if (/lender summary|covenant schedule|whole.?loan|debt yield/i.test(text)) signals.real_estate_debt += 3;
  if (/\bdscr\b|loan.?to.?value|senior loan|debt service/i.test(text)) signals.real_estate_debt += 3;
  if (signals.real_estate_debt > 0 && signals.re_fund > 0) signals.real_estate_debt += signals.re_fund; // it IS real estate, just lender-side
...
function detectOutputs(gt, sheets) {
    ...
    if (/exit.*multiple|ebitda.*multiple|cap.*rate|revenue.*multiple|arr.*multiple|exit.*yield|terminal.*yield/.test(lower)) {
      ...
      if (rowMatch) {
        ...
        if (best) {
          ...
          exitMultCands.push({ cell: best.addr, isCapRate, score });
        }
      }
    }
  }
  ...
  if (exitMultCands.length > 0) {
    ...
    outputs.exitMultiple = { cell: win.cell, type: win.isCapRate ? 'cap_rate_inverse' : 'ebitda_multiple' };
  }
...
function detectDebt(gt, sheets) {
  ...
  for (const [addr, val] of Object.entries(gt)) {
    if (typeof val !== 'string') continue;
    const lower = val.toLowerCase();
    if (!/exit.*debt|debt.*exit|exit.*balance|loan.*balance/.test(lower)) continue;
    const bang = addr.lastIndexOf('!');
    const sheet = addr.substring(0, bang);
    const rowMatch = addr.match(/(\d+)$/);
    if (!rowMatch) continue;
