...
function detectDebt(gt, sheets) {
  ...
  for (const [addr, val] of Object.entries(gt)) {
    ...
    // Reject leverage/coverage RATIO cells masquerading as a dollar balance. A
    // label like "Exit Leverage (Debt/EBITDA)" matches /exit.*debt/ but is a
    // turns-of-EBITDA multiple (~1.8), not the closing dollar balance — binding it
    // rendered "Debt at exit: $2", a hard trust-kill for a lender. Genuine balance
    // labels never carry these tokens.
    if (/leverage|\bratio\b|multiple|coverage|debt\s*\/\s*ebitda|x\s*ebitda|\bturns\b/.test(lower)) continue;
    const bang = addr.lastIndexOf('!');
    const sheet = addr.substring(0, bang);
    const rowMatch = addr.match(/(\d+)$/);
    if (!rowMatch) continue;
