 ...
 */
function pickFundNav(gt, timeline, rvpiAddr, paidInAddr) {
  ...
  for (const [addr, val] of Object.entries(gt)) {
    if (typeof val !== 'string') continue;
    const lower = val.toLowerCase();
    if (!navRe.test(lower) && !residRe.test(lower)) continue;
    const bang = addr.lastIndexOf('!');
    const sheet = addr.substring(0, bang);
    const rowMatch = addr.match(/(\d+)$/);
    if (!rowMatch) continue;
    const row = parseInt(rowMatch[1], 10);
    const best = pickRightmostInRange(gt, sheet, row, 'residualValue');
    if (!best) continue;
    ...
    candidates.push({ addr: best.addr, value: best.value, isNav: navRe.test(lower), tlCount });
  }
  if (candidates.length === 0) return null;
  ...
  const score = (c) => {
    let s = 0;
    if (cols.length >= 3) s += (c.tlCount < 3 ? 100 : 0); // a scalar beats a cashflow series row
...
function detectFundLevelMetrics(gt, sheets, timeline) {
  ...
  const _nav = pickFundNav(gt, timeline, fund.rvpi, fund.paidIn);
